...
void ImGui::BeginPieMenuEx( bool released )
{
	...
	if( !s_oPieMenuContext.m_released )
...
void ImGui::EndPieMenuEx( void )
{
	IM_ASSERT(s_oPieMenuContext.m_iCurrentIndex >= 0);
	PieMenuContext::PieMenu& oPieMenu = s_oPieMenuContext.m_oPieMenuStack[s_oPieMenuContext.m_iCurrentIndex];
	--s_oPieMenuContext.m_iCurrentIndex;
}
bool ImGui::BeginPiePopup(const char* pName, ImVec2 center, int iMouseButton, bool released)
{
	if (ImGui::IsPopupOpen(pName))
	{
		...
		s_oPieMenuContext.m_released = released;
...
void ImGui::EndPiePopup()
{
	...
	for (int iIndex = 0; iIndex <= s_oPieMenuContext.m_iMaxIndex; ++iIndex)
	{
		...
		float fMenuHeight = sqrtf(oPieMenu.m_fMaxItemSqrDiameter);
...
bool ImGui::BeginPieMenu(const char* pName, bool bEnabled)
{
...
void ImGui::EndPieMenu()
{
	IM_ASSERT(s_oPieMenuContext.m_iCurrentIndex >= 0 && s_oPieMenuContext.m_iCurrentIndex < PieMenuContext::c_iMaxPieItemCount);
	--s_oPieMenuContext.m_iCurrentIndex;
}
bool ImGui::PieMenuItem(const char* pName, bool bEnabled)
{
	IM_ASSERT(s_oPieMenuContext.m_iCurrentIndex >= 0 && s_oPieMenuContext.m_iCurrentIndex < PieMenuContext::c_iMaxPieItemCount);
	PieMenuContext::PieMenu& oPieMenu = s_oPieMenuContext.m_oPieMenuStack[s_oPieMenuContext.m_iCurrentIndex];
	ImVec2 oTextSize = ImGui::CalcTextSize(pName, NULL, true);
